...
describe('RewilderNFT', function () {
    ...
    describe('upgrades', function() {
        beforeEach(async function () {
            const RewilderNFT = await ethers.getContractFactory('RewilderNFT');
            this.token = await upgrades.deployProxy(RewilderNFT, { kind: 'uups' , initializer: 'initialize'});
        });

        it('upgrades to v2 implementation and preserves address', async function () {
            const MockRewilderNFTv2 = await ethers.getContractFactory('MockRewilderNFTv2');
            const upgradedNFT = await upgrades.upgradeProxy(this.token.address, MockRewilderNFTv2);
